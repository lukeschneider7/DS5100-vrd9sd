...
# Student Info
    ...
    Name: Luke Schneider
    Net ID: vrd9sd
    File GitHub URL: https://github.com/lukeschneider7/DS5100-vrd9sd/blob/main/lessons/M11/M11-HW-2.Rmd 
...
# CODE HERE
library(tidyverse)
abalone_data = read.csv(file='https://archive.ics.uci.edu/ml/machine-learning-databases/abalone/abalone.data', header=FALSE)
head(abalone_data)
```
...
```{r}
# CODE HERE
infant_data = abalone_data %>% filter(V1== 'I')
print(nrow(infant_data))
```
...
```{r}
# CODE HERE
infant_male_data = abalone_data %>% filter(V1== 'I' | V1 == 'M')
print(nrow(infant_male_data))
```
...
```{r}
# CODE HERE
abalone_data %>% group_by(V1) %>% summarize(mean(V2))
```
...
```{r}
# CODE HERE
abalone_data %>% group_by(V1) %>% filter(V1=='M') %>% summarize(median(V2))
```
