...
Name: Luke Schneider
Ned ID: vrd9sd 
URL of this file on GitHub: https://github.com/lukeschneider7/DS5100-vrd9sd/blob/main/lessons/M10/M10-HW-1-1.Rmd 
```
...
```{r}
# CODE Here
quota_percent = c(40, 30, 20, 10)
rates = c(7, 10, 13, 16)
df = data.frame(quota_percent=quota_percent, rates = rates) # initial data frame

quota = 225000
df['quarterly_quota'] = cumsum(quota * (df$quota_percent/100)) # cumulative quota
df["earnings"] = (df$quota_percent/100) * quota * rates/100 # earnings for each bracket
df['quota_percentages'] = c(0, .4, .7, .9) # quota percentages reached at start of each bracket
df["pay"] = cumsum(df$earnings) # cumulative earnings 
head(df[, 1:6])
```
...
```{r}
# CODE HERE
amount_earned = function(fraction_of_quota){
    earned = 0
    if (fraction_of_quota > .9){
      earned = df$pay[3] + ((fraction_of_quota - df$quota_percentages[4])/.1) *(df$earnings[4])
    }
    else if (fraction_of_quota > .7){
      earned = df$pay[2] + ((fraction_of_quota - df$quota_percentages[3])/.2) *(df$earnings[3])
    }
    else if (fraction_of_quota > .4){
      earned = df$pay[1] + ((fraction_of_quota - df$quota_percentages[2])/.3) *(df$earnings[2])
    }
    else{
      earned = ((fraction_of_quota - df$quota_percentages[1])/.4) * (df$earnings[1])
    }
...
amount_earned(.8)
print(amount_earned(.4))
print(amount_earned(.7))
print(amount_earned(.9))
print(amount_earned(1))
...
# CODE HERE
df2 <- data.frame()
s1 <- seq(from=0, to=1, by=.1)
for (percent in s1){
  payout <- amount_earned(percent)
  new_row <- data.frame(percent_quota_earned = c(percent*100), 
             payout = c(payout))
...
# CODE HERE
par(mfrow = c(1, 2))
plot(df$quarterly_quota, df$pay, xlab = "quarterly quota ($)", ylab = "amount earned ($)", main = "Amount earned over quota")
plot(df$quarterly_quota, df$pay, type='l', xlab = "quarterly quota ($)", ylab = "amount earned ($)", main = "Amount earned over quota")
...
# CODE HERE
par(mfrow = c(1, 2))
plot(df2$percent_quota_earned*(quota/100), df2$payout, xlab = "quarterly quota ($)", ylab = "payout earned ($)", main = "Payout over quota")
plot(df2$percent_quota_earned*(quota/100), df2$payout, type='l', xlab = "quarterly quota ($)", ylab = "payout earned ($)", main = "Payout over quota")
