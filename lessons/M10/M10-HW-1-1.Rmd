...
rates = c(7, 10, 13, 16)
df = data.frame(quota_percent=quota_percent, rates = rates)

quota = 225000
df['quarterly_quota'] = quota * (df$quota_percent/100)
df['quarterly_quota'] = cumsum(df['quarterly_quota'])
df["earnings"] = (df$quota_percent/100) * quota * rates/100
df['quota_percentages'] = c(0, .4, .7, .9)
df["pay"] = cumsum(df$earnings)
df
```
...
```{r}
# CODE HERE
plot(df$quarterly_quota, df$pay)
plot(df$quarterly_quota, df$pay, type='l')
